...
import { normalizeImageUrl, DEFAULT_LOGO } from '../utils/imageHelper';
...
export default function PublicMicrositePage({ site: initialSite, onGoHome }) {
  ...
  // 1. Real-time Cloud & Local Storage Subscription
  useEffect(() => {
    ...
    // Fetch data terkini dari Cloud / URL Parameter / Local Cache
    ...
    // BroadcastChannel Listener untuk sinkronisasi instan antar-tab
    let channel;
    try {
  ...
  // Merge Priority: Cloud Data -> Initial Passed Site -> Matching Official Preset -> Baseline
  ...
  // 2. Record page view on mount (Unconditionally declared)
  useEffect(() => {
    if (activeSlug) {
  ...
  // Loading state while resolving live cloud data (AFTER all hooks)
  ...
  // Single Clean Title & Profile Resolution
  const profile = {
    ...DEFAULT_MICROSITE_DATA.profile,
    ...(matchingPreset?.data?.profile || {}),
    ...(rawData.profile || {}),
    title: rawData.profile?.title || currentSite.title || rawData.profile?.departmentName || rawData.profile?.universityName || 'UNIVERSITAS PELITA BANGSA'
  };
  ...
  const handleCopyLink = () => {
    const origin = typeof window !== 'undefined' && window.location.origin.includes('localhost') 
      ...
      : 'https://pmbupb.site';
    const fullSite = { ...currentSite, data: mergedRendererData };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#040914] text-slate-100 relative overflow-x-hidden">
      
      {/* Floating Top Navigation Bar */}
      <header className="relative z-30 max-w-md mx-auto w-full px-4 pt-3 pb-1 flex items-center justify-between">
        <button
          onClick={onGoHome}
          className="px-3 py-1.5 bg-black/50 hover:bg-black/70 backdrop-blur-md border border-white/15 rounded-xl text-xs font-bold text-white flex items-center gap-1.5 transition shadow-sm"
        >
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="p-1.5 bg-black/50 hover:bg-black/70 backdrop-blur-md border border-white/15 rounded-xl text-white transition shadow-sm"
      ...
      {/* Identical Microsite Renderer as seen in Studio Dasbor Phone Preview */}
      <main className="flex-1 flex flex-col">
        <MicrositeRenderer
          data={mergedRendererData}
