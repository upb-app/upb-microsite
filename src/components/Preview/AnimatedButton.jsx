...
export default function AnimatedButton({ 
  ...
  const animationClass = link.animation && link.animation !== 'inherit' 
    ? link.animation 
    : (globalButtonStyle.animation || 'anim-hover-scale');

  // Determine button shape & styling
  const roundedClass = globalButtonStyle.rounded || 'rounded-xl';
  const variant = globalButtonStyle.variant || 'glass';

  // Base styles based on variant
  let styleClasses = "";
  let inlineStyles = {};

  if (link.customBgColor) {
    inlineStyles.backgroundColor = link.customBgColor;
    inlineStyles.color = link.customTextColor || '#ffffff';
    inlineStyles.borderColor = link.customBorderColor || 'transparent';
  } else {
    switch (variant) {
      case 'solid':
      ...
      case 'outline':
        ...
        break;
      case 'gradient':
        styleClasses = "bg-gradient-to-r from-upb-800 to-upb-950 text-white border border-upb-500/30 hover:from-upb-700 hover:to-upb-900 shadow-md";
        break;
      case 'soft':
  ...
  return (
    <a
      ...
      className={`group relative flex items-center justify-between w-full p-3.5 transition-all duration-300 ${roundedClass} ${styleClasses} ${animationClass} ${
        link.highlight ? 'ring-2 ring-amber-400/80 shadow-lg shadow-amber-500/20' : ''
      }`}
    >
